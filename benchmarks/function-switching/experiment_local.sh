...
run_benchmark() {
    ...
    ${BUILD_DIR}/orchestrator/orchestrator ${SCRIPT_DIR}/config/orchestrator_${mode}.json ../devices.json > orchestrator_output.log 2>&1 &
    orchestrator_pid=$!
...
for device_config in "${device_configs[@]}"; do
	...
	#srun --oversubscribe -n 1 --jobid=${GPU_JOB} 
  ${REPO_DIR}/tools/list-gpus.sh . ${GPU_ID}
