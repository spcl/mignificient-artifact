/*
  ...
  Modified by adding CHECK_LAST_CUDA_ERROR and the kernel function: raiseError
  that triggers fatal gpu error during kernel execution.
*/
...
#define CHECK_LAST_CUDA_ERROR(index) checkLast((index), __FILE__, __LINE__)
void checkLast(int index, const char* const file, const int line)
{
...
int main(int argc, char** argv)
{
    ...
    printf("cudaDevAttrMultiProcessorCount(SMs): %d\n", devProp.multiProcessorCount);
    ...
    scalar=3.0f;
    // int valid = 120;
    for (k=start_index; k<NTIMES; k++)
    {
        // if (valid == 0) {
        //     std::cout << "throw error" << std::endl;
        //     raiseError<<<dimGrid,dimBlock>>>();
        //     std::cout << "error at " << k << std::endl;
        //     return (0);
        // }

        times[0][k]= mysecond();
        STREAM_Copy<real><<<dimGrid,dimBlock>>>(d_a, d_c, N);
        // CHECK_LAST_CUDA_ERROR(k); // newly added
