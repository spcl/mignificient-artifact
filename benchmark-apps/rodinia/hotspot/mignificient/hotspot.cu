...
extern "C" int function(mignificient::Invocation invoc)
{
  ...
  run(
    input_json["grid-rows"].asInt(),
    input_json["pyramid-height"].asInt(),
    input_json["total-iterations"].asInt(),
    input_json["tfile"].asString().c_str(),
    input_json["pfile"].asString().c_str(),
    input_json["ofile"].asString().c_str()
  );
